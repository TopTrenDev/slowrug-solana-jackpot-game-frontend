    ...
    <div className="min-h-screen">
      <div className="relative h-40 sm:h-48 overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 px-4 pb-4 sm:px-8 sm:pb-6 lg:px-12">
          ...
          <h1 className="font-display text-4xl sm:text-5xl text-foreground lg:text-6xl">THE TOWER</h1>
          <p className="mt-1 text-xs sm:text-sm text-muted-foreground">Climb higher. Multiply bigger. Cash out before collapse.</p>
        </div>
      </div>

      <div className="px-4 py-6 sm:px-8 sm:py-8 lg:px-12">
        {/* Controls first on mobile for better UX */}
        <div className="grid gap-4 sm:gap-6 lg:grid-cols-[1fr_360px]">
          {/* Controls Panel - shown first on mobile via order */}
          <div className="space-y-4 order-1 lg:order-2">
            ...
            {playing && !crashed && (
              <div className="border border-primary/30 bg-primary/5 p-4 sm:p-6 neon-glow-green animate-slide-up">
                <p className="font-mono text-[10px] font-semibold tracking-widest text-muted-foreground uppercase">Multiplier</p>
                <p className="mt-1 font-mono text-4xl sm:text-5xl font-black text-primary neon-text-green">{multiplier}x</p>
          ...
          {/* Tower Visualization */}
          <div className="border border-border bg-card/60 backdrop-blur-sm p-4 sm:p-6 scanlines relative overflow-hidden order-2 lg:order-1">
            {crashed && (
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-background/90 backdrop-blur-sm animate-fade-blur">
                <span className="font-display text-4xl sm:text-6xl text-destructive neon-text-red mb-2">COLLAPSED!</span>
            ...
            <div className="flex flex-col-reverse gap-1.5 sm:gap-2">
              {Array.from({ length: LEVELS }, (_, i) => i + 1).map((level) => {
                ...
                return (
                  <div
                    key={level}
                    className={`relative flex items-center justify-between border px-3 py-2.5 sm:px-5 sm:py-3.5 transition-all duration-500 ${
                      isCleared
                  ...
                  >
                    <div className="flex items-center gap-2 sm:gap-3">
                      <span className={`flex h-6 w-6 sm:h-7 sm:w-7 items-center justify-center font-mono text-[10px] sm:text-xs font-bold ${
                        isCleared ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground"
                      }`}>
                        {level}
                      </span>
                      <span className={`font-medium text-xs sm:text-sm tracking-wide ${isCleared ? "text-primary" : "text-muted-foreground"}`}>
                        Level {level}
                      </span>
                    </div>
                    <span className={`font-mono text-xs sm:text-sm font-bold ${isCleared ? "text-primary neon-text-green" : "text-muted-foreground"}`}>
